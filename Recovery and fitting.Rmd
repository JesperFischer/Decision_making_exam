...
for (s in 1:123){
  #in trials
  for (t in 1:307){
    sa2[t,s] = mean(sa22[1:3000,t,s])
  }
}

colnames(sa2) = ids$X266


datty = data.frame(sa2)

datty = datty %>% pivot_longer(cols = everything())

datty$name = sub('.', '', datty$name)

datty$id = datty$name

datty$name = NULL

datty$id = as.factor(datty$id)

datty = datty %>% group_by(id) %>% mutate(trial = 1:307)

datty = datty %>% filter(trial != 307)

df1$id = as.factor(df1$id)

datty2 = inner_join(datty, df1, by = c("id","trial"))


dataaaa = datty2 %>% dplyr::select("burnbeta","sa2","stim","vasRT_3","trial","id") %>% drop_na()
...
summary(expectation_sa2_burn_HGF)
```


```{r}

cleanu = df1 %>% dplyr::select(u,id,trial) %>% drop_na()


cleanu = cleanu %>% pivot_wider(names_from = c("id"), values_from = c("u"))


cleany = df1 %>% dplyr::select(predResp2,id,trial) %>% drop_na()
...
samples <- jags.parallel(data, inits=NULL, params,
                         model.file ="/home/jespere/Decision_making_exam/HHGF.txt", n.chains=3, 
                         n.iter=3000, n.burnin=1000)
...
sa22 = samples$BUGSoutput$mean$sa2

sa22 = as.data.frame(sa22)

colnames(sa22) = ids

sa22 = sa22 %>% pivot_longer(cols = everything())

sa22 = sa22 %>% group_by(name) %>% mutate(trial = 1:306)

sa22$id = sa22$name
sa22$name = NULL
sa22$sa22 = sa22$value

gg = inner_join(df1,sa22,by = c("id","trial"))


dataaaa = gg %>% dplyr::select("vasResp_3","sa22","sa2","stim","vasRT_3","trial","id") %>% drop_na()

dataaaa$value = dataaaa$vasResp_3/100
dataaaa$id = as.factor(dataaaa$id)

library(gamlss)
expectation_sa2_burn_HGF <-gamlss(value ~ sa22*stim+vasRT_3+trial+random(id),
           nu.formula = ~ sa22*stim+vasRT_3+trial+random(id),
           tau.formula = ~ sa22*stim+vasRT_3+trial+random(id),
           data = dataaaa,
           family = BEINF(mu.link = "logit",sigma.link = "logit",nu.link = "logit",tau.link = "logit"),trace = F)
